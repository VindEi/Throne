...
namespace Configs
{
    SubUserInfo ParseSubUserInfo(const QString &info) {
        ...
        static const QRegularExpression webUrlRe(R"((?:^|[;\s])web_url=([^;\n\s]+))", QRegularExpression::CaseInsensitiveOption);
        static const QRegularExpression supportUrlRe(R"((?:^|[;\s])support_url=([^;\n\s]+))", QRegularExpression::CaseInsensitiveOption);
        static const QRegularExpression announceRe(R"((?:^|[;\s])announce=([^;\n]+))", QRegularExpression::CaseInsensitiveOption);
    ...
    {
        QMutexLocker locker(&mutex);
        if (profiles.size() <= idx1 || profiles.size() <= idx2)
            return false;
    ...
    {
        QMutexLocker locker(&mutex);
        if (profiles.size() <= idx || profiles.size() <= newIdx)
            return false;
